...
while True:
    try:
        number1, operation, number2 = [
            i for i in input('Введите математическое вырожание через пробел в виде  (число операнд число): ').split()
        ]
    except ValueError:
        print('Невернный ввод вырожения используйте (+,-,*,/) и для завершения 0 ')
        continue
    number1 = int(number1)
    number2 = int(number2)

    if operation == '0':
        print('Заверщение вычислений')
        break
    elif operation == '+':
        ...
        print('Неверное вырожение используйте (+,-,*,/) и для завершения 0 ')
